...
async function loadWords(){
  try{
    const parts=await Promise.all(LEVEL_FILES[level].map(async path=>{const r=await fetch(path);if(!r.ok)throw new Error(path);return parseCsv(await r.text())}));
    words=parts.flat().map(w=>({...w,id:Number(w.id),level:Number(w.level)}));
    if(!words.length)throw new Error("词库为空");
    loadState();startSavedMode();hideError();
  }catch(err){showError(`词库读取失败：${err.message}`)}
}

function loadState(){
  try{state={...defaultState(),...JSON.parse(localStorage.getItem(stateKey())||"{}")}}catch{state=defaultState()}
...
function startSavedMode(){
  if(state.studyOrder.length!==words.length||!state.studyOrder.every(id=>words.some(w=>w.id===id))){state.studyOrder=allIds();state.studyCursor=0}
  mode="study";queue=[...state.studyOrder];cursor=Math.min(state.studyCursor,queue.length);render();
}
...
function render(){
  updateStats();revealed=false;ui.answer.classList.add("hidden");ui.ratingButtons.classList.add("hidden");ui.revealBtn.classList.remove("hidden");ui.donePanel.classList.add("hidden");ui.card.classList.remove("hidden");
  if(cursor>=queue.length){renderDone();return}
  const w=currentWord();if(!w){renderDone();return}
...
function updateStats(){
  const completed=Object.keys(state.ratings).filter(id=>Number(id)<=words.at(-1).id).length;
  const mastered=words.filter(w=>state.ratings[w.id]==="know").length;
  const review=reviewIds().length;
  ui.progressText.textContent=`${Math.min(completed,words.length)} / ${words.length}`;ui.masteredText.textContent=mastered;ui.reviewText.textContent=review;ui.progressBar.style.width=`${words.length?completed/words.length*100:0}%`;
}
...
function rate(value){
  if(!revealed||cursor>=queue.length)return;
  const id=queue[cursor];state.ratings[id]=value;cursor++;
  if(mode==="study")state.studyCursor=cursor;else state.reviewCursor=cursor;
  saveState();render();
}
function renderDone(){
  ui.card.classList.add("hidden");ui.ratingButtons.classList.add("hidden");ui.donePanel.classList.remove("hidden");const count=reviewIds().length;
  ui.doneTitle.textContent=mode==="review"?"复习完成":"本轮完成";
  ui.doneText.textContent=count?`还有 ${count} 个词在复习库。":"当前档位没有待复习单词。";
...
ui.revealBtn.addEventListener("click",reveal);ui.card.addEventListener("click",e=>{if(e.target===ui.card||e.target===ui.wordText||e.target===ui.phoneticText)reveal()});ui.card.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();reveal()}});
document.querySelectorAll(".rating").forEach(b=>b.addEventListener("click",()=>rate(b.dataset.rating)));
ui.reviewBtn.addEventListener("click",startReview);ui.restartBtn.addEventListener("click",()=>startStudy(true));ui.resetBtn.addEventListener("click",resetCurrent);
loadWords();
